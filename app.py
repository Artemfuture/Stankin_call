...
def create_room():
    ...
    # Инициализация комнаты: список участников и модератор
...
def on_join(data):
    ...
    # Отправляем обновлённый список пользователей всем в комнате
    emit(
        "user_joined",
        {
            "username": username,
...
def handle_toggle_track(data):
    ...
    emit(
        "toggle_track",
        {"target": target_username, "type": track_type, "enabled": enabled},
        room=room_id,
...
def handle_kick_user(data):
    ...
    if room_info and room_info["moderator"] == moderator_username:
        ...
        room_info["users"] = [
            u for u in room_info["users"] if u["username"] != target_username
        ]
